...
export function ListsDropdown({ isOpen, onClose, triggerRef }: ListsDropdownProps) {
  ...
  return (
    <div
      ref={dropdownRef}
      className={cx(
        "absolute start-full top-0 z-[60] ms-2 w-48 rounded-lg border border-gray-200 bg-white shadow-lg  ",
  ...
  return (
    <div>
      <button
        onClick={onToggle}
        className={cx(
          "flex w-full items-center justify-between rounded-md px-2 py-1.5 text-sm font-medium transition-colors hover:bg-gray-100 ",
          focusRing
        )}
