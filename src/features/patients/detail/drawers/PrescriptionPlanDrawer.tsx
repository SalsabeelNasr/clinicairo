...
import type { PatientPrescription, PrescriptionFormPayload } from "../patient-prescription.types"
import {
  formatPrescriptionFileSize,
  prescriptionFileFromUpload,
} from "../prescription-file.utils"

interface PrescriptionPlanDrawerProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  prescription: PatientPrescription | null
  onSubmit: (payload: PrescriptionFormPayload) => Promise<void>
}

export function PrescriptionPlanDrawer({
  open,
  onOpenChange,
  prescription,
  onSubmit,
}: PrescriptionPlanDrawerProps) {
  ...
  const handleFileChange = (file: File | undefined) => {
    ...
    if (file.type !== "application/pdf" && !file.name.toLowerCase().endsWith(".pdf")) {
      setSelectedFile(null)
      setFileError(t.profile.prescriptionPdfOnly)
      return
    }
    setFileError(null)
  ...
  const handleSubmit = async () => {
    if (selectedFile) {
      setSaving(true)
      try {
        await onSubmit(prescriptionFileFromUpload(selectedFile))
        onOpenChange(false)
      } finally {
        setSaving(false)
      }
  ...
  const canSubmit = prescription ? Boolean(selectedFile || prescription) : Boolean(selectedFile)
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent side={isRtl ? "left" : "right"} className="w-full sm:max-w-lg">
        <DrawerHeader>
          <DrawerTitle>
            {prescription ? t.profile.editPrescriptionTitle : t.profile.addPrescriptionTitle}
          </DrawerTitle>
        </DrawerHeader>
        <DrawerBody className="space-y-4">
              ...
              <div className="min-w-0">
                ...
                {prescription.file_size != null && (
                  <p className="mt-0.5 text-theme-xs text-gray-500" dir="ltr">
                    {formatPrescriptionFileSize(prescription.file_size)}
                  </p>
                )}
                <p className="mt-1 text-theme-xs text-gray-500">
                  {t.profile.prescriptionReplaceFileHint}
                </p>
              </div>
            </div>
          )}
          ...
          <div>
            <Label>{t.profile.prescriptionPdfUploadLabel}</Label>
            <input
              ref={inputRef}
              type="file"
            ...
            >
              ...
              <span className="text-theme-sm font-medium text-gray-700">
                {selectedFile ? selectedFile.name : t.profile.prescriptionChoosePdf}
              </span>
              {selectedFile && (
                <span className="text-theme-xs text-gray-500" dir="ltr">
                  {formatPrescriptionFileSize(selectedFile.size)}
                </span>
              )}
              <span className="text-theme-xs text-gray-500">{t.profile.prescriptionPdfOnly}</span>
            </button>
            {fileError && (
